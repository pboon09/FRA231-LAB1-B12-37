...
void rt_OneStep(void)
{
  ...
  __enable_irq();
  currentTime = (extmodeSimulationTime_T) Homing_M->Timing.taskTime0;
  Homing_step();
...
int main(int argc, char **argv)
{
  ...
  errorCode = extmodeInit(Homing_M->extModeInfo, &rtmGetTFinal(Homing_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(Homing_M, true);
    }
  }
