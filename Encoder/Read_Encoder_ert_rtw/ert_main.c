...
void rt_OneStep(void)
{
  ...
  __enable_irq();
  currentTime = (extmodeSimulationTime_T) Read_Encoder_M->Timing.taskTime0;
  Read_Encoder_step();
...
int main(int argc, char **argv)
{
  ...
  MX_TIM4_Init();
  MX_TIM8_Init();
  rtmSetErrorStatus(Read_Encoder_M, 0);
  ...
  Read_Encoder_initialize();
  __disable_irq();
  __enable_irq();

  /* External Mode initialization */
  errorCode = extmodeInit(Read_Encoder_M->extModeInfo, &rtmGetTFinal
    (Read_Encoder_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(Read_Encoder_M, true);
    }
  }
  ...
  runModel =
    ...
    !rtmGetStopRequested(Read_Encoder_M);
  __enable_irq();
  __enable_irq();
  while (runModel) {
